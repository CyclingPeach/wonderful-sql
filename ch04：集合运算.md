...
在标准 SQL 中, 分别对检索结果使用 `UNION`, `INTERSECT,` `EXCEPT` 来将检索结果进行 `并、交、差运算`, 像 `UNION`,`INTERSECT`, `EXCEPT` 这种用来进行 `集合运算` 的运算符称为 `集合运算符` 。
...
在数据库中, 所有的 `表` 以及 `查询结果` 都可以视为 `集合` , 因此也可以把 `表` 视为 `集合` 进行上述 `集合运算` , 在很多时候, 这种抽象非常有助于对复杂查询问题给出一个可行的思路。
