...
为了便于理解，称之为 `窗口函数`。常规的SELECT语句都是对整张表进行查询，而窗口函数可以让我们有选择的对某一部分数据进行汇总、计算和排序。
...
* 窗口函数 OVER 中的 **ORDER BY 子句**并不会影响最终结果的排序。其**只是用来决定窗口函数按何种顺序计算**。
# 5.4 GROUPING运算符
